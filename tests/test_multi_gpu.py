...
@needs_cuda
@needs_multiple_gpus
def test_create_data_parallel_lstm(lstm: ContextualLSTM):
...
@needs_cuda
@needs_multiple_gpus
def test_data_parallel_returns_same_result_as_serial(lstm: ContextualLSTM, glove_embeddings: WordEmbeddings):
    encoding = [
        torch.tensor([0, 1, 5, 8, 3, 1], dtype=torch.long, device=lstm.device),
        torch.tensor([1, 4, 6, 1, 9, 7], dtype=torch.long, device=lstm.device),
        torch.tensor([9, 0, 6, 9, 9, 0], dtype=torch.long, device=lstm.device),
    ...
    serial = ContextualLSTM(glove_embeddings, device="cuda")
